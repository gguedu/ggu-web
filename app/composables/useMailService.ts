...
export const useMailService = () => {
  ...
  const accountSetName = (accountId: number, name: string) =>
    request<void>('/account/setName', { method: 'PUT', body: { accountId, name } });
  ...
  const emailRead = (emailIds: number[] | number) =>
    request<void>('/email/read', {
      method: 'PUT',
      body: { emailIds: Array.isArray(emailIds) ? emailIds : [emailIds] },
    });
  const starAdd = (emailId: number) =>
    request<void>('/star/add', { method: 'POST', body: { emailId } });
  const starCancel = (emailId: number) =>
    request<void>('/star/cancel', { method: 'DELETE', params: { emailId } });
  ...
  const settingSet = (payload: Record<string, unknown>) =>
    request<void>('/setting/set', { method: 'PUT', body: payload });
  const resetPassword = (password: string) =>
    request<void>('/my/resetPassword', { method: 'PUT', body: { password } });
