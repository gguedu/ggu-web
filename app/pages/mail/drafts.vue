...
<template>
  <section class="p-6 text-white">
    <div class="flex items-center justify-between mb-4">
      ...
      <button
        class="text-sm px-3 py-2 rounded bg-blue-600 hover:bg-blue-500"
    ...
    <div v-else class="space-y-3">
      <article
        ...
        :key="item.draftId"
        class="rounded-xl border border-[#333] bg-[#1c1c1e] p-4"
      >
        <div class="flex items-start justify-between gap-3">
          <div class="min-w-0 flex-1 cursor-pointer" @click="editDraft(item.draftId)">
            ...
            <div class="text-xs text-gray-500 mt-2 truncate">
              收件人: {{ item.receiveEmail.join(',') || '未填写' }}
            </div>
            <div class="text-xs text-gray-500 mt-1">最后编辑: {{ formatDate(item.updatedAt) }}</div>
          </div>

          <button
            class="p-1 rounded hover:bg-[#2b2b2e] text-red-300"
            @click="removeDraft(item.draftId)"
          >
