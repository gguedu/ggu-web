  ...
  <section class="p-6 text-white">
    ...
    <template v-else>
      <div class="flex items-center gap-2 mb-5">
        ...
        <button class="px-2 py-1 rounded hover:bg-[#2c2c2e]" @click="toggleStar">
          {{ email.isStar ? '取消星标' : '设为星标' }}
        </button>
        <button class="px-2 py-1 rounded hover:bg-[#2c2c2e]" @click="goReply">回复/转发</button>
        <button class="px-2 py-1 rounded text-red-300 hover:bg-red-900/30" @click="removeEmail">
          删除
        </button>
      </div>
      ...
      <article class="rounded-xl border border-white/10 bg-white/5 backdrop-blur-sm p-5">
        ...
        <div class="grid gap-1 text-sm text-gray-300 mb-4">
          ...
          <div>
            时间: {{ email.createTime ? new Date(email.createTime).toLocaleString('zh-CN') : '-' }}
          </div>
        </div>

        <div class="border-t border-white/10 pt-4">
          <div v-if="email.content" class="prose prose-invert max-w-none" v-html="email.content" />
          <pre v-else class="whitespace-pre-wrap text-sm text-gray-200">{{
            email.text || '(空内容)'
          }}</pre>
        </div>
        ...
        >
          ...
          <ul class="space-y-2 text-sm">
            ...
            >
              ...
              <a
                v-if="att.key"
                class="text-blue-300 hover:text-blue-200 ml-3"
                :href="att.key"
                target="_blank"
                >下载</a
              >
